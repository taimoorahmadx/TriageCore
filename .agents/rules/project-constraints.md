...
- **PROACTIVE DOCUMENTATION (MANDATORY & UNCONDITIONAL)**: You must proactively, automatically, and unconditionally keep project documentation (`MILESTONES.md`, `README.md`, docs, etc.) strictly up to date after completing ANY code change, bug fix, task, or milestone. Never wait for the user to ask or remind you to document changes. In `MILESTONES.md`, always update the AI Usage Log with exact, detailed records of *what* was changed, *why* it was changed, and *what previous state* it replaced, and automatically check off `[ ]` to `[x]` for any completed Definition of Done items.
