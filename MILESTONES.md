...
- **Technical Spec:** A tiny, standalone Python script using Playwright that visits a real page, clicks a button, and when the button's ID is manually changed by a human, uses an LLM to relocate the button via semantic DOM matching and completes the click.
- **Agent Prompt:** "Create a standalone Playwright script `poc_recovery.py`. It should load a target URL and click a known button. If the selector fails, extract the surrounding DOM, send it to the LLM to find the new selector based on semantic context, and retry the click. Do NOT invent business logic. Catch Playwright TimeoutErrors explicitly."
- **Definition of Done:** 
...
- **Agent Prompt:** "Create `src/browser/runner.py` with `run_test` and `extract_context`. On Playwright failure, extract the immediate parent/child DOM of the failed selector and its structural position. Return this as structured JSON. Handle `playwright.TimeoutError` and `playwright.TargetClosedError` gracefully."
- **Definition of Done:**
  - [ ] 3 failing test scripts correctly trigger extraction without crashing.
  - [ ] Extracted JSON matches the expected structure.
- **Human Tasks:** Write target HTML pages. Ensure Playwright installs correctly.
- **AI Usage Log Template:** 
...
- **Human Tasks:** Push broken code to dummy repo to generate real logs.
- **AI Usage Log Template:** 
...
- **Agent Prompt:** "Create `tests/benchmark.py` to loop through a directory of 15 JSON files, pass them to the agents, and output a CSV of expected vs actual classifications. Update `docker-compose.yml` to run the QA and CI agents alongside Postgres and FastAPI."
- **Definition of Done:**
...
- **Human Tasks:** Finalize the 10-15 real-world baseline cases. Provision the cloud VM and run the deploy.
- **AI Usage Log Template:** 
...
- **Agent Prompt:** "Add GitHub API logic to `ci_agent.py` to open a PR. SAFETY RULE: Never merge, only open. The PR body MUST strictly format the `confidence_score` and `reasoning_trace` from the engine output. Implement a FastAPI Jinja2 dashboard for the `triage_decisions` table."
- **Definition of Done:**
